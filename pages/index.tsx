...
export default function Home() {
  ...
  const [resultVal, setResultVal] = useState<string>('text-[#b5cea8] text-[18px]')
    ...
    <div>
      <div className='p-[16px] font-[Consolas,_"Courier_New",_monospace]'>
        <button
          ...
          onClick={() => copyText(resultVal)}
          disabled={!demoEnded}
        >
          Copy Result Code
        </button>
        <p className='text-[#b5cea8] text-[18px] leading-[30px] mt-[16px]'>
          <span className='text-[#abb2bf] font-bold'>Result Code: </span>
          <span className='bg-[#1e1e1e] p-[6px_10px]'>{resultVal}</span>
        </p>
      </div>
      <Editor
        language='css'
